...
| `CLIENT_FORCE_SPEAKER` | `1` | phone browsers: play the voice on the **loudspeaker**, not the earpiece (Android Chrome flips to ear-style 'communication' routing while the mic is live; iOS gets a WebAudio fallback). Mobile-UA only — desktop/headphones untouched. **Served to `/studio` via `GET /client/ice-config`** and applied in its `connect()`. It used to be a `/client`-only `<head>` patch, which `MUSETALK_SPLIT=1` made INERT — so this knob did nothing at all until 2026-07-14. `0` = off (`docs/PROBLEMS-AND-FIXES.md` P24) |
