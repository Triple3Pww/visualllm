...
def analyze(wav_bytes):
    """Return (duration_s, longest_silence_s, silence_fraction) from 16-bit PCM wav."""
    ...
    for i in range(0, len(a) - win, win):
        ...
        if peak < thresh:
            ...
            cur = 0
    return dur, longest * win / sr, silent_wins * win / sr / dur if dur else 0
...
def main():
    ...
    for i in range(1, args.runs + 1):
        ...
        dur, longest_sil, sil_frac = analyze(wav)
        durs.append(dur)
        ...
        print(f"  run{i}: dur={dur:5.2f}s  longest_silence={longest_sil:4.2f}s  "
              f"silence_frac={sil_frac:0.2f}{flag}")
